...
const defaultCategories: Category[] = [
  {
    id: uuidv4(),
    name: "Laptop",
    subcategories: ["Gaming", "Office", "Ultrabook"],
  },
...
const defaultProducts: Product[] = [
  {
    ...
    description:
      "Laptop gaming ringan dengan performa buas. Dilengkapi prosesor AMD Ryzen 9 dan GPU NVIDIA RTX 3060.",
...
export const addSubcategory = (categoryId: string, subcategoryName: string) => {
  ...
  if (
    index !== -1 &&
    !categories[index].subcategories.includes(subcategoryName)
  ) {
...
export const removeSubcategory = (
  ...
  const index = categories.findIndex((c) => c.id === categoryId)
  if (index !== -1) {
    categories[index].subcategories = categories[index].subcategories.filter(
      (s) => s !== subcategoryName,
    )
    localStorage.setItem(CATEGORIES_KEY, JSON.stringify(categories))
  }
...
export const login = (): boolean => {
  localStorage.setItem(
    USER_KEY,
    JSON.stringify({ username: "admin", isLoggedIn: true }),
  )
  return true
}
